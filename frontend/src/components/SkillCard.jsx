...
const SkillCard = ({ skill }) => {
  ...
  // Get the first image from mediaUrls or fallback to image property
  const displayImage = mediaUrls.length > 0 ? mediaUrls[0].url : image;
  ...
  return (
    <Link to={`/skill/${_id || skill.id}`} className="block">
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-lg hover:border-gray-300 transition-all duration-200 cursor-pointer">
        {/* Image */}
        {displayImage && (
        ...
        <div className="p-6">
          ...
          {/* Action Button */}
          <div className="w-full bg-primary-600 text-white py-2.5 px-4 rounded-lg hover:bg-primary-700 transition-colors font-medium text-center">
